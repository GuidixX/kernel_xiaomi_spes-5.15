...
static int dwc3_core_init(struct dwc3 *dwc)
{
	...
	/*
	 * Above DWC_usb3.0 1.94a, рекомендуется выставлять DWC3_GUSB3PIPECTL_SUSPHY и DWC3_GUSB2PHYCFG_SUSPHY в '0' при конфигурировании coreConsultant. Значение по умолчанию будет '0' после сброса ядра. Приложение должно выставить '1' после завершения инициализации ядра.
	 *
	 * Некоторые phy требуют состояния P0 во время инициализации. Убедитесь, что GUSB3PIPECTL.SUSPENDENABLE и GUSB2PHYCFG.SUSPHY сброшены перед инициализацией phy для поддержания состояния P0.
	 *
	 * После инициализации phy некоторые операции возможны только в низких P-состояниях. Убедитесь, что GUSB3PIPECTL.SUSPENDENABLE и GUSB2PHYCFG.SUSPHY выставлены сразу после инициализации, чтобы не блокировать phy-операции.
	 */
